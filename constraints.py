...
class UnitCellFilter_symm(Filter):
    def __init__(self, atoms,final, mask=None,
                 cell_factor=None,
                 hydrostatic_strain=False,
                 constant_volume=False):
    ...
    def get_forces(self, apply_constraint=False):
        ...
        s1, s6, s5= forces[len(atoms)+0]
        s6, s2, s4= forces[len(atoms)+1]
        s5, s4, s3= forces[len(atoms)+2]
        ...
        forces[len(atoms)+0]= tensor[0,:]
        forces[len(atoms)+1]= tensor[1,:]
        forces[len(atoms)+2]= tensor[2,:]
        ###
        return forces
