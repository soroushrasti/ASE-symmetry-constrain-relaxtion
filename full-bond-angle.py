...
f = open("ice-ii.order.data", "w")       
with open("ice-ii.data") as f3:
    for line in f3:
        line  = line.rstrip('\n')
        if "atoms" in line:
             atoms=line.rstrip('\n').split(" ")
...
oxygen=1
hydrogen=2
mol= 99999*np.ones(atomnumber)
out = np.zeros(atomnumber)
charge= np.zeros(atomnumber)
data = np.genfromtxt("ice-ii.data", skip_header=numberline, max_rows=atomnumber)
      ...
      for j in range(0,atomnumber):
           if out[j]==1:
                ...
                if z[i]-z[j]> zpard/2:
                      ...
                      if dis[i][j] < bondlenght:
                           break
                if z[i]-z[j] > zpard/2 and x[i]-x[j]> xpard/2:
                      dis[i][j] = math.sqrt((y[j] - y[i])**2 + (x[j] - (x[i]-xpard))**2 + (z[j] - (z[i]-zpard))**2)
                      if dis[i][j] < bondlenght:
                           break
                if -z[i]+z[j] > zpard/2 and x[i]-x[j]> xpard/2:
                      dis[i][j] = math.sqrt((y[j] - y[i])**2 + (x[j] - (x[i]-xpard))**2 + (z[j] - (z[i]+zpard))**2)
                ...
                if z[i]-z[j] > zpard/2 and -x[i]+x[j]> xpard/2:
                      ...
                      if dis[i][j] < bondlenght:
                           break
                if z[i]-z[j] > zpard/2 and y[i]-y[j]> ypard/2:
                      dis[i][j] = math.sqrt((y[j] - (y[i]-ypard))**2 + (x[j] - (x[i]-xypard))**2 + (z[j] - (z[i]-zpard))**2)
                      if dis[i][j] < bondlenght:
                           break
                if -z[i]+z[j] > zpard/2 and y[i]-y[j]> ypard/2:
                      dis[i][j] = math.sqrt((y[j] - (y[i]-ypard))**2 + (x[j] - (x[i]-xypard))**2 + (z[j] - (z[i]+zpard))**2)
                ...
                if z[i]-z[j] > zpard/2 and -y[i]+y[j]> ypard/2:
                      ...
                      if dis[i][j] < bondlenght:
                           break
                if x[i]-x[j] > xpard/2 and y[i]-y[j]> ypard/2:
                      dis[i][j] = math.sqrt((y[j] - (y[i]-ypard))**2 + (x[j] - (x[i]-xypard-xpard))**2 + (z[j] - z[i])**2)
                      if dis[i][j] < bondlenght:
                           break
                if -x[i]+x[j] > xpard/2 and y[i]-y[j]> ypard/2:
                      dis[i][j] = math.sqrt((y[j] - (y[i]-ypard))**2 + (x[j] - (x[i]-xypard+xpard))**2 + (z[j] - z[i])**2)
